...
bool Navigation::moveToGoalWithTimeout(double xGoal, double yGoal, double phiGoal,
                                       int timeout_sec) {
	if (!action_client_->action_server_is_ready()) {
		RCLCPP_ERROR(node_->get_logger(), "Action server not available");
		return false;
	...
	RCLCPP_INFO(node_->get_logger(), "Sending goal (%.2f, %.2f, %.2f) [timeout: %ds]...",
	            xGoal, yGoal, phiGoal, timeout_sec);
	...
	if (rclcpp::spin_until_future_complete(node_, result_future, timeout) !=
	    rclcpp::FutureReturnCode::SUCCESS) {
		RCLCPP_WARN(node_->get_logger(), "Goal timed out after %ds — cancelling", timeout_sec);
	...
	RCLCPP_WARN(node_->get_logger(), "Goal did not succeed (code %d)",
